...
def dd_train(args):
    ...
    if mod == "vgg16":
        ...
        gamma = 0.03
        beta = 0.05
        model_file = "/projects/synergy_lab/ayush/dense_weights_ddnet_vgg16/dense_weights/weights_dense_" + str(
            epochs) + ".pt"
    elif mod == "vgg19":
        ...
        gamma = 0.04
        beta = 0.04
        model_file = "/projects/synergy_lab/ayush/dense_weights_ddnet_vgg19/dense_weights/weights_dense_" + str(
            epochs) + ".pt"
    else:
    ...
    for k in range( epochs +  retrain):
        ...
        if enable_profile:
            ...
            train_total, train_mse, train_msi, train_vgg, val_total, val_mse, val_msi, val_vgg = \
                 _epoch_vgg(model, train_index_list, val_index_list, train_loader, val_loader, scheduler, optimizer, scaler, sched_type)
            train_total_loss[k] = train_total
            train_MSE_loss[k] = train_mse
            train_MSSSIM_loss[k] = train_msi
